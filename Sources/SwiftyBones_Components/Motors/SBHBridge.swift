...
struct SBHBridge: SBComponentOutProtocol {
    ...
    func setRawValue(value: Int) -> Bool {
        ...
        if !gpioEnable.setValue(newValue) {
            return false
        }
        return true
    }
    ...
    */
    func goForward() -> Bool {
        if let gpioForward = self.gpioForward, gpioReverse = self.gpioReverse {
            gpioForward.setValue(DigitalGPIOValue.HIGH)
            gpioReverse.setValue(DigitalGPIOValue.LOW)
            return true
        }
        return false
    }
    ...
    func goReverse() -> Bool {
        if let gpioForward = self.gpioForward, gpioReverse = self.gpioReverse {
            gpioForward.setValue(DigitalGPIOValue.LOW)
            gpioReverse.setValue(DigitalGPIOValue.HIGH)
            return true
        }
        return false
    }
